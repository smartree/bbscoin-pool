            ...
            async.filter(transferCommands, function(transferCmd, cback){
                apiInterfaces.rpcWallet('sendTransaction', transferCmd.rpc, function(error, result){
                    if (error){
                        ...
                        log('error', logSystem, 'Payments failinged to send to %j', transferCmd.rpc.transfers);
                        cback(false);
                        return;
                    }
                    ...
                    for (var i = 0; i < transferCmd.rpc.transfers.length; i++){
                        var transfer = transferCmd.rpc.transfers[i];
                        transferCmd.redis.push(['zadd', config.coin + ':payments:' + transfer.address, now, [
                            txHash,
                            transfer.amount,
                            transferCmd.rpc.fee,
                            transferCmd.rpc.anonymity
